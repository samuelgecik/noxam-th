				...
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					...
					<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
						<div className="rounded-2xl overflow-hidden shadow-md bg-white border border-gray-100">
							...
							<div className="p-5">
								<h3 className="font-bold text-gray-900">
									{t("ref_warehouse_title")}
								</h3>
								<p className="text-sm text-gray-500 mt-1">
									{t("ref_warehouse_desc")}
								</p>
							</div>
						</div>
						<div className="rounded-2xl overflow-hidden shadow-md bg-white border border-gray-100">
			...
			<section className="py-16 bg-white">
				<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
					...
					<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
						{([
							{ key: "1", initials: "SK", color: "bg-primary" },
							{ key: "2", initials: "NP", color: "bg-green-600" },
							{ key: "3", initials: "WL", color: "bg-purple-600" },
						] as const).map((item) => (
							<div key={item.key} className="p-6 rounded-xl bg-gray-50 border border-gray-100">
								...
								<p className="text-gray-600 mb-6">
									&quot;{t(`testimonial${item.key}_text`)}&quot;
								</p>
								<div className="flex items-center gap-3">
									<div className={`flex size-12 items-center justify-center rounded-full ${item.color} text-white font-bold`}>
										{item.initials}
									</div>
									<div>
										<h4 className="font-bold text-gray-900">
											{t(`testimonial${item.key}_name`)}
										</h4>
										<p className="text-sm text-gray-500">
											{t(`testimonial${item.key}_role`)}
										</p>
									</div>
								</div>
							</div>
					...
					<div className="grid lg:grid-cols-2 gap-12 items-center">
						<div>
							...
							<div className="space-y-4">
								{([
									{ key: "certified", icon: "school" },
									{ key: "finnish", icon: "engineering" },
									{ key: "equipment", icon: "precision_manufacturing" },
								] as const).map((item) => (
									<div key={item.key} className="flex items-start gap-4">
										...
										<div>
											<h4 className="font-bold text-gray-900">
												{t(`training_${item.key}_title`)}
											</h4>
											<p className="text-sm text-gray-600">
												{t(`training_${item.key}_desc`)}
											</p>
										</div>
									</div>
								))}
